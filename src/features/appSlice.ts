...
export const createOrder = createAsyncThunk(
	'app/createOrder',
	async (
		data: { ingredients: string[]; token: string },
		{ rejectWithValue }
	) => {
		try {
			const response = await API.createOrder(data.ingredients, data.token);
			return response;
		} catch (error: any) {
...
export const appSlice = createSlice({
	...
	reducers: {
		addIngredientToConstructor: (
			state,
			action: PayloadAction<SelectedIngredient>
		) => {
		...
		setIngredients(state, action: PayloadAction<Ingredient[]>) {
			...
			state.ingredientsMap = action.payload.reduce((acc, ingredient) => {
